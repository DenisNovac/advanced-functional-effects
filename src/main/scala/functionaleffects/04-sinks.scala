/** In ZIO Streams, Sinks are consumers of values. Not only do they consume values, but they are free to produce
  * remainders ("leftovers"), as well as terminate with values of a given type (or error).
  *
  * Sinks are the duals of streams. While streams emit values, sinks absorb them. To create a complete pipeline, a
  * stream must be connected to a sink, because values need a place to go. All the run methods of Stream are actually
  * implemented by connecting streams to sinks and running the complete pipeline.
  */
    ...
    suite("Constructors") {
      ...
      test("count") {
        ...
        /** EXERCISE
          ...
          * increments the ref by each value consumed by the sink.
          */
        test("foreach") {
          val stream = ZStream(1, 2, 3, 4)
        ...
        /** EXERCISE
          ...
          * multiplies all consumed values together (producing their product).
          */
        test("foldLeft") {
          val stream = ZStream(1, 2, 3, 4)
...
object Operators extends ZIOSpecDefault {
  def spec =
    suite("Operators") {
      ...
      /** EXERCISE
        *
        * Use the `.map` method of the provided `sink` to map the collected values into the length of the collected
        * values. Then replace the call to `.runCount` with a call to `.run(sink)`.
        */
      test("map") {
...
/** GRADUATION
  *
  * To graduate from this section, you will implement a sink that writes results to a persistet queue.
  */
